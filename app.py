...
CORE_CARBON_BOUNDARY = "Core construction-stage energy carbon"
EXTENDED_CARBON_BOUNDARY = "Extended construction-stage energy carbon"

CORE_CARBON_DEFINITION = """
This is the construction-stage energy boundary used for the main comparison. It includes:

- direct emissions from diesel burned by site plant and generators;
- emissions from generating the grid electricity used for plant charging, temporary power and site-battery recharging.

It excludes upstream diesel supply, electricity transmission and distribution, and upstream electricity emissions.
"""

EXTENDED_CARBON_DEFINITION = """
This includes everything in the core boundary, plus:

- upstream emissions from extracting, refining and delivering diesel;
- electricity transmission and distribution;
- upstream emissions associated with supplying electricity.

It is a broader construction-stage energy boundary, but it is not a whole-life carbon assessment.
"""
...
def make_plot_mobile_friendly(
    ...
    *,
    ...
    # Only format a Plotly title when the figure genuinely has title text.
    # Passing title formatting without title text can render the word
    # "undefined" in some mobile browsers.
    ...
    if existing_title:
        ...
        # Plotly may render an empty title object as "undefined" on some
        # mobile browsers. An explicit blank text value avoids that bug.
...
with section_d:
    ...
    st.markdown("**Carbon boundary**")
    st.caption("Select one boundary. Click or tap each ? for its definition.")

    if "carbon_boundary" not in st.session_state:
        st.session_state.carbon_boundary = EXTENDED_CARBON_BOUNDARY

    def select_carbon_boundary(selected_boundary: str) -> None:
        st.session_state.carbon_boundary = selected_boundary

    core_button, core_help = st.columns([12, 1])
    with core_button:
        st.button(
            CORE_CARBON_BOUNDARY,
            key="select_core_carbon_boundary",
            type=(
                "primary"
                if st.session_state.carbon_boundary == CORE_CARBON_BOUNDARY
                else "secondary"
            ),
            use_container_width=True,
            on_click=select_carbon_boundary,
            args=(CORE_CARBON_BOUNDARY,),
        )
    with core_help:
        with st.popover("?", help=f"Definition of {CORE_CARBON_BOUNDARY}"):
            st.markdown(f"**{CORE_CARBON_BOUNDARY}**")
            st.markdown(CORE_CARBON_DEFINITION)

    extended_button, extended_help = st.columns([12, 1])
    with extended_button:
        st.button(
            EXTENDED_CARBON_BOUNDARY,
            key="select_extended_carbon_boundary",
            type=(
                "primary"
                if st.session_state.carbon_boundary == EXTENDED_CARBON_BOUNDARY
                else "secondary"
            ),
            use_container_width=True,
            on_click=select_carbon_boundary,
            args=(EXTENDED_CARBON_BOUNDARY,),
        )
    with extended_help:
        with st.popover("?", help=f"Definition of {EXTENDED_CARBON_BOUNDARY}"):
            st.markdown(f"**{EXTENDED_CARBON_BOUNDARY}**")
            st.markdown(EXTENDED_CARBON_DEFINITION)

    boundary = st.session_state.carbon_boundary
    st.caption(f"Selected boundary: {boundary}")

    if boundary == CORE_CARBON_BOUNDARY:
...
with section_f:
    ...
    st.latex(r"CO_{2e,core}=L_{diesel,total}\,EF_{diesel,direct}")
    st.latex(r"CO_{2e,extended}=L_{diesel,total}(EF_{diesel,direct}+EF_{diesel,upstream})")
    ...
    st.markdown("**Electric core and extended construction-stage energy carbon**")
    st.latex(r"CO_{2e,core}=E_{grid,total}EF_{grid,effective}+L_{backup,total}EF_{diesel,direct}")
    st.latex(r"CO_{2e,extended}=E_{grid,total}(EF_{grid,effective}+EF_{grid,T\&D}+EF_{grid,upstream})+L_{backup,total}(EF_{diesel,direct}+EF_{diesel,upstream})")
    st.warning(
